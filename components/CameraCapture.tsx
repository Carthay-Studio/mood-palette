...
const CameraCapture: React.FC<CameraCaptureProps> = ({ onImageSelected, preview }) => {
  ...
  if (stream) {
      return (
        <div className="flex flex-col w-full h-full gap-4">
            <div className="relative w-full h-64 border-2 border-border bg-black overflow-hidden group">
  ...
  if (preview) {
      return (
        <div className="relative w-full h-full border-2 border-border bg-bg overflow-hidden group cursor-pointer" onClick={startCamera}>
  ...
  return (
       <div
        onClick={startCamera}
        className="flex flex-col items-center justify-center w-full h-full border-2 border-border bg-bg hover:bg-surface hover:border-text-muted transition-all duration-500 cursor-pointer group"
      >
        {error ? (
          <p className="text-status-error font-mono text-xs uppercase tracking-widest text-center max-w-xs px-4">{error}</p>
        ) : (
