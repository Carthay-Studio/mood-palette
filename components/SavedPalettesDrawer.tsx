...
const SavedPalettesDrawer: React.FC<SavedPalettesDrawerProps> = ({ isOpen, onClose, palettes, onLoad, onDelete, buildNumber }) => {
        ...
        <div className="flex-1 overflow-y-auto p-12 space-y-12">
          {palettes.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-modal-border opacity-50">
                 <span className="font-mono text-xs uppercase tracking-widest text-center">Void<br/>No Data</span>
            </div>
          ) : (
              ...
              <div key={palette.name} className="group relative animate-fade-up">
                <div className="border-2 border-modal-border p-8 bg-white hover:border-modal-text transition-colors duration-700 ease-carthay">
                    <div className="flex justify-between items-start mb-8">
                        <h3 className="font-product text-3xl font-light tracking-wide text-modal-text break-words max-w-[70%]">{palette.name}</h3>
                        <div className="flex gap-2 shrink-0">
                            {/* Action Buttons */}
                            <button onClick={(e) => { e.stopPropagation(); handleShare(palette); }} className="text-modal-border hover:text-modal-text transition-colors" title="Copy Link">
                                <ShareIcon className="w-4 h-4" />
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); handleDownload(palette); }} className="text-modal-border hover:text-modal-text transition-colors" title="Download ASE">
                                <DownloadIcon className="w-4 h-4" />
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); onDelete(palette.name); }} className="text-modal-border hover:text-status-error transition-colors" title="Delete">
