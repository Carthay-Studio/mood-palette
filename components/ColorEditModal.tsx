...
const ColorEditModal: React.FC<ColorEditModalProps> = ({ isOpen, onClose, name, hex, colorMode, onSave }) => {
    const [editName, setEditName] = useState(name);
    const [editHex, setEditHex] = useState(hex);
    ...
    const displayedValue = () => {
        switch(colorMode) {
            case 'RGB': return hexToRgb(editHex);
            case 'HSL': return hexToHsl(editHex);
            case 'CMYK': return hexToCmyk(editHex);
            default: return editHex.toUpperCase();
        }
    };
            ...
            <div className="relative bg-modal-bg border-2 border-modal-border text-modal-text w-full max-w-md p-8 md:p-12 animate-fade-up shadow-2xl">
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-2">
                        <label className="font-mono text-[10px] uppercase tracking-widest text-gray-500 block">Color Identity</label>
                        <input 
                            type="text"
                            value={editName}
